...
interface ModalCheckInProps {
  ...
  onConfirm: (operadorEntrada: string, descricaoServico: string) => Promise<void>;
}

const OPERADORES_CCO = ['CCO - Thiago', 'CCO - Roberto', 'CCO - Amanda', 'CCO - Bruno'];

export default function ModalCheckIn({ isOpen, onClose, colaborador, onConfirm }: ModalCheckInProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      const savedOperador = localStorage.getItem('ultimo_operador_cco_entrada') || '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!operador) {
      setError('Por favor, selecione o operador do CCO responsável.');
      return;
    }
    ...
    setSubmitting(true);
    try {
      // Salvar preferência de operador
      localStorage.setItem('ultimo_operador_cco_entrada', operador);
      await onConfirm(operador, descricao.trim());
      onClose();
    } catch (err: any) {
      ...
      <div className="w-full max-w-lg glass-panel rounded-3xl overflow-hidden shadow-2xl border border-[rgba(255,26,60,0.2)]">
        ...
        <div className="px-8 py-6 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(255,26,60,0.03)] flex justify-between items-center">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[var(--accent-red)] text-[24px]">login</span>
            <h3 className="text-[18px] font-black tracking-[0.5px] text-white">LIBERAÇÃO DE ENTRADA</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[12px] font-bold uppercase tracking-[1px] text-slate-400">Operador CCO Responsável</label>
            <select
              value={operador}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[12px] font-bold uppercase tracking-[1px] text-slate-400">Descrição do Serviço (Texto Aberto)</label>
            <textarea
              value={descricao}
              onChange={(e) => setDescricao(e.target.value)}
              placeholder="Descreva detalhadamente o serviço a ser executado hoje..."
